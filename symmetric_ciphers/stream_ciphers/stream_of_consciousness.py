...
# Co robi serwer?
# Ma listę jakiś słów z których jedno to flaga
# Jak callniemy endpoint to wybiera losowo jedno słowo i wrzuca do CTR jako plaintext klucz jest jeden to jest ważne
# Jak tak sobie postrzelamy w endpoint kilkanaście razy to jesteśmy w stanie uzbierać ileś distrinct ciphertextów
#
...
# tesutejmy czy ten key działa też na tym ciphertext
def xor_all(ciphers, test_key):
    for cipher in ciphers:
        cipher = bytes.fromhex(cipher)
        for i in range(len(test_key)):
            if i >= len(cipher): break # koniec pętli
            a = test_key[i] ^ cipher[i] # jeśli to co wyjdzie z XOR'a klucza i ciphertext ...
            if not (a > 31 and a < 127): # nie jest znakiem plaintextowym znakiem ascii to mówimy ze klucz lipny
                return False
            #print(chr(a), end='')
        #print()
...
def load_ciphers(): #otwiera plik zebranych ciphertext i bierze je do listy
    ciphers = set()
    with open('symmetric_ciphers/stream_ciphers/ciphers.txt', 'rb') as fd:
        while True:
...
def find_key_and_flag():
    prefix = b'crypto{' #wiemy ze flaga sie od tego zaczyna
    key = [] # zaczynamy z pustym kluczem
    encrypted_flag = b''
    for c in ciphers:
        c = bytes.fromhex(c) # bierzemy ciphertext
        k = []
        for i in range(len(prefix)):
            k.append(prefix[i] ^ c[i]) # konkatenujemy do k bajt który jest XORem 'crypto' i początku ciphertextu
        if xor_all(ciphers, k):  # jeśli ten kawałek klucza jest legit na każdym szyfrze to jest to legit kawałek klucza
            print('found', k, len(k)) # znaleźliśmy legitny początek klucza
            key[:] = k[:]
            encrypted_flag = c
            break

        if key: break # jeśli znaleźliśmy key to przerwij pętle
    return key, encrypted_flag # zwraca kawałek klucza i kawałek flagi

#dump_ciphers()
ciphers = load_ciphers()
key, encrypted_flag = find_key_and_flag()

# w tym momencie mamy kawałek klucza (o taki [218, 23, 122, 170, 172, 29, 205]) i flagi teraz będzie zgadywać kolejne znaki klucza
# przujmuje jakiś szyfr, kawałek kliucza i guess na następny znak
...
# tutaj dajemy ciphertext i key (czyli flagę i aktualny klucz) i patrzymy co zostanie odkodowane czyli patrzymy jak wygląda flaga
...
# No i teraz tak: trzeba po kolei patrzeć czy te zdania co się budują mają sens naprzykład wychodzi mi 'Three boa' albo 'As if I pw<" no to trzeba podmieniać i inaczej zgadywać klucz
# dużo tu manualnej roboty
...
# nie wiem czy to najlepsza metoda na zadanie