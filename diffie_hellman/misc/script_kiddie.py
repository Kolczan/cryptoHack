from Crypto.Cipher import AES
import hashlib
import secrets

# Ten skrypt to generalnie jest uruchamiany po stornie Bob'a, który dostaje od Alice p,g,A
# Potem skrypt wymyśla b (losowanko) i B (robi to swoją XORową zamiast potęgową metodą)

def header():
    print("""  _____  _  __  __ _
 |  __ \(_)/ _|/ _(_)
 | |  | |_| |_| |_ _  ___
 | |  | | |  _|  _| |/ _ \
 | |__| | | | | | | |  __/
 |_____/|_|_| |_| |_|\___|
 | |  | |    | | |
 | |__| | ___| | |_ __ ___   __ _ _ __
 |  __  |/ _ \ | | '_ ` _ \ / _` | '_ \
 | |  | |  __/ | | | | | | | (_| | | | |
 |_|  |_|\___|_|_|_| |_| |_|\__,_|_| |_|

                                        """)


def is_pkcs7_padded(message):
    padding = message[-message[-1]:]
    return all(padding[i] == len(padding) for i in range(0, len(padding)))


def pkcs7_unpad(message, block_size=16):
    if len(message) == 0:
        raise Exception("The input data must contain at least one byte")
    if not is_pkcs7_padded(message):
        return message
    padding_len = message[-1]
    return message[:-padding_len]


def decrypt_flag(shared_secret: int, iv: str, ciphertext: str):
    # Derive AES key from shared secret
    sha1 = hashlib.sha1()
    sha1.update(str(shared_secret).encode('ascii'))
    key = sha1.digest()[:16]
    # Decrypt flag
    ciphertext = bytes.fromhex(ciphertext)
    iv = bytes.fromhex(iv)
    cipher = AES.new(key, AES.MODE_CBC, iv)
    plaintext = cipher.decrypt(ciphertext)
    return pkcs7_unpad(plaintext).decode('ascii')


def generate_public_int(g, a, p):
    # zamiast A = pow(g,a,p)
    return g ^ a % p


def generate_shared_secret(A, b, p):
    return A ^ b % p


def goodbye():
    print('Goodbye!')


def main():
    header()
    print('[-] Collecting data from Alice')
    p = int(input('> p: '))
    q = (p - 1) // 2
    g = int(input('> g: '))
    A = int(input('> A: '))
    print('[+] All data collected from Alice')

    print('[+] Generating public integer for alice')
    b = secrets.randbelow(q)
    B = generate_public_int(g, b, p)
    print(f'[+] Please send the public integer to Alice: {B}')
    print('')
    input('[+] Press any key to continue')
    print('')

    print('[+] Generating shared secret')
    # tutaj jest retrieve `a` - `a` to shared secret (secret, którym Alice sie z nami podzieliła)
    shared_secret = generate_shared_secret(A, b, p)

    query = input('Would you like to decrypt a message? (y/n)\n')
    if query == 'y':
        iv = input('[-] Please enter iv (hex string)\n')
        ciphertext = input('[-] Please enter ciphertext (hex string)\n')
        flag = decrypt_flag(shared_secret, iv, ciphertext)
        print(f'[+] Flag recovered: {flag}')
        goodbye()
    else:
        goodbye()

def script_kiddie():
    p = 
    g = 2
    A = 
    B = 

    iv = 'c044059ae57b61821a9090fbdefc63c5'
    encrypted_flag = 'f60522a95bde87a9ff00dc2c3d99177019f625f3364188c1058183004506bf96541cf241dad1c0e92535564e537322d7'

    # Mamy juz prawie wszystko potrzebuje tylko `b`, żeby jako shared_secret wsadzić do `decrypt_flag()` (jak w poprzednich zadaniach)
    # `b` widzac po tym skrypcie otrzymam z funkcji `b = generate_shared_secret()`, której podam ... np. bedąc Alice B,a,p a będąc Bob A,b,p
    # szukam `b`, więc wybieram ze jestem Alice, skąd teraz wziąć `a`?
    # zastnanówmy się jak zostało ono wygenerowane po stronie Alice ..... no zostało wylosowane xd
    # Ale czy mamy gdzieś jakiś ślad po tym `a`?
    # Tak otóż podczas generowania `A` - w funkcji `generate_public_int()`
    # widać ze jest tam XOR'owanie, zamiast potegowania
    # A kto uważał na lab1 to wie z właściwości XOR, że może zapisać poniższą równość i jest ona prawdziwa:
    a = g ^ A
    # w ten oto sposób mamy `a`

    #warto non stop patrzeć na `dh_starter4.py`

    b = generate_shared_secret(B, a, p)

    print(decrypt_flag(b, iv, encrypted_flag))